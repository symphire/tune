...
fn main() {
    ...
    for i in 0..2 {
        ...
        let app_state: Rc<RefCell<dyn AppState>> = Rc::new(RefCell::new(RealAppState::new(message_tx.clone(), network)));
        ...
        client.app_state.borrow_mut().update(AppMessage::CaptchaRequest(captcha_key));
        ...
        client.app_state.borrow_mut().update(AppMessage::SignupRequest(SignupInput {
        ...
        client.app_state.borrow_mut().update(AppMessage::LoginRequest(LoginInput {
            username: client.username.clone(),
            password: client.password.clone(),
            captcha_id: CaptchaId(uuid::Uuid::nil()),
            captcha_answer: captcha.clone(),
        ...
        client.app_state.borrow_mut().update(AppMessage::FriendListRequest);
        if let Ok(app_message) = client.message_rx.recv() {
            trace!("friend list event: {:?}", app_message);
            client.app_state.borrow_mut().update(app_message);
        }
        client.app_state.borrow_mut().update(AppMessage::EstablishConnectionRequest);
        if let Ok(app_message) = client.message_rx.recv() {
    ...
    clients[0].app_state.borrow_mut().update(AppMessage::AddFriendRequest(clients[1].username.clone()));
    ...
    if let Ok(app_message) = clients[0].message_rx.recv() {
        trace!("friend list received (left={}): {:?}", clients[0].message_rx.len(), app_message);
        clients[0].app_state.borrow_mut().update(app_message);
    }
    ...
    clients[0].app_state.borrow_mut().update(AppMessage::OpenConversation(conv.unwrap()));
    if let Ok(app_message) = clients[0].message_rx.recv() {
        trace!("history received: {:?}", app_message);
        clients[0].app_state.borrow_mut().update(app_message);
    }

    clients[0].app_state.borrow_mut().update(AppMessage::ChatMessageRequest(ChatMessageInput {
    ...
    debug!("history: {:?}", clients[0].app_state.borrow().get_conversation_history(conv.unwrap()));
    ...
    clients[1].app_state.borrow_mut().update(AppMessage::OpenConversation(conv.unwrap()));
    if let Ok(app_message) = clients[1].message_rx.recv() {
        trace!("history received: {:?}", app_message);
        clients[1].app_state.borrow_mut().update(app_message);
    }
    debug!("history: {:?}", clients[1].app_state.borrow().get_conversation_history(conv.unwrap()));

    clients[1].app_state.borrow_mut().update(AppMessage::ChatMessageRequest(ChatMessageInput {
        conversation_id: conv.unwrap(),
        message_id: MessageId(uuid::Uuid::new_v4()),
